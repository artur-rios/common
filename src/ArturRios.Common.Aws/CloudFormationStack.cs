...
public class CloudFormationStack(Construct scope, string id, IStackProps props) : Stack(scope, id, props)
{
    ...
    {
        var condition = _conditions.GetOrAdd(conditionKey,
            key => new CfnCondition(this, key, new CfnConditionProps { Expression = expression }));
    ...
    {
        var mapping = _mappings.GetOrAdd(mapName,
            _ => new CfnMapping(this, mapName,
                new CfnMappingProps { Mapping = new ConcurrentDictionary<string, IDictionary<string, object>>() }));
