...
{
    List<RestApiResourceMethod> _methods  = new();
    
    public ApiGatewayRestApi Api { get; }
    public RestApiResource Parent { get; }
    
    public RestApiResource(string pathPart, ApiGatewayRestApi api) : base(api, pathPart, new CfnResourceProps
    {
        PathPart = pathPart,
        ParentId = api.Ref,
        RestApiId = api.AttrRootResourceId
    })
    {
        Api = api;
    }
    
    public RestApiResource(string pathPart, RestApiResource parent) : base(parent, pathPart, new CfnResourceProps
    {
        PathPart = pathPart,
        ParentId = parent.Ref,
        RestApiId = parent.Api.Ref
    })
    {
        Api = parent.Api;
        Parent = parent;
